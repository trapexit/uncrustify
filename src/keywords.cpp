...
static const chunk_tag_t *kw_static_match(bool orig_list, const chunk_tag_t *tag, int lang_flags)
{
   bool in_pp = (  cpd.in_preproc != E_Token::CT_NONE
...
E_Token find_keyword_type(const char *word, size_t len)
{
   ...
   return((p_ret != nullptr) ? p_ret->type : E_Token::CT_WORD);
} // find_keyword_type
...
int load_keyword_file(const char *filename)
{
   ...
   while (fgets(buf, max_line_length, pf) != nullptr)
   {
      ...
      if (argc > 0)
      {
         ...
         {
            add_keyword(args[0], E_Token::CT_TYPE);
         }
         else
         {
...
void print_custom_keywords(FILE *pfile)
{
   for (const auto &keyword_pair : dkwm)
   {
      ...
      else if (tt == E_Token::CT_MACRO_NO_FMT_ARGS)
      {
...
pattern_class_e get_token_pattern_class(E_Token tok)
{
   ...
   {
   case E_Token::CT_IF:
   case E_Token::CT_ELSEIF:
   case E_Token::CT_SWITCH:
   case E_Token::CT_FOR:
   case E_Token::CT_WHILE:
   case E_Token::CT_SYNCHRONIZED:
   case E_Token::CT_USING_STMT:
   case E_Token::CT_LOCK:
   case E_Token::CT_D_WITH:
   case E_Token::CT_D_VERSION_IF:
   case E_Token::CT_D_SCOPE_IF:
      return(pattern_class_e::PBRACED);

   case E_Token::CT_ELSE:
      return(pattern_class_e::ELSE);

   case E_Token::CT_DO:
   case E_Token::CT_TRY:
   case E_Token::CT_FINALLY:
   case E_Token::CT_BODY:
   case E_Token::CT_UNITTEST:
   case E_Token::CT_UNSAFE:
   case E_Token::CT_VOLATILE:
   case E_Token::CT_GETSET:
      return(pattern_class_e::BRACED);

   case E_Token::CT_CATCH:
   case E_Token::CT_D_VERSION:
   case E_Token::CT_DEBUG:
      return(pattern_class_e::OPBRACED);

   case E_Token::CT_NAMESPACE:
      return(pattern_class_e::VBRACED);

   case E_Token::CT_WHILE_OF_DO:
      return(pattern_class_e::PAREN);

   case E_Token::CT_INVARIANT:
